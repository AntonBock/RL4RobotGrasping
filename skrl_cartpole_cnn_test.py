...
class Policy(GaussianModel):
    def __init__(self, observation_space, action_space, device, clip_actions=False,
        ...
        self.net_img = nn.Sequential(nn.Conv2d(1, 4, kernel_size=8, stride=4),
                                 nn.ReLU(),
        ...
        self.net_fc = nn.Sequential(nn.Linear(36, 32),
                                    nn.ReLU(),
    ...
    def compute(self, states, taken_actions):
        ...
        img, data = torch.split(states, [64*64, 4], dim=1)

        # print(f"observation space: {self.observation_space.shape}")
        # view (samples, width * height * channels) -> (samples, width, height, channels) 
        # permute (samples, width, height, channels) -> (samples, channels, width, height) 
        x = self.net_img(img.view(-1, 64, 64, 1).permute(0, 3, 1, 2))
...
if tt:
    ...
    networks_ppo["policy"].load("./runs/cnn_cart/checkpoints/1400_policy.pt") 
